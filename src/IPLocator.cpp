    ...
    const unsigned char *addr)
{
    ...
    {
        // LOG_WARN(IP_LOCATOR, "Trying to set an IPv4 in a non IPv4 Locator");
        return false;
    }
    ...
    const std::string &ipv4)
{
    ...
    if (ss >> a >> ch >> b >> ch >> c >> ch >> d)
    {
        ...
        locator.address[12] = (unsigned char)a;
        locator.address[13] = (unsigned char)b;
        locator.address[14] = (unsigned char)c;
        locator.address[15] = (unsigned char)d;

        // If there are more info to read, it fails
        return ss.rdbuf()->in_avail() == 0;
    }
    // LOG_WARN(IP_LOCATOR, "IPv4 " << ipv4 << " error format. Expected X.X.X.X");
    return false;
}
...
const unsigned char *IPLocator::getIPv4(
    const Locator &locator)
{
    return static_cast<const unsigned char *>(&locator.address[12]);
}
    ...
    const unsigned char *addr)
{
    ...
    {
        // LOG_WARN(IP_LOCATOR, "Trying to set an IPv6 in a non IPv6 Locator");
        return false;
    }
    ...
    uint16_t group7)
{
    ...
    locator.address[0] = (unsigned char)(group0 >> 8);
    locator.address[1] = (unsigned char)group0;
    locator.address[2] = (unsigned char)(group1 >> 8);
    locator.address[3] = (unsigned char)group1;
    locator.address[4] = (unsigned char)(group2 >> 8);
    locator.address[5] = (unsigned char)group2;
    locator.address[6] = (unsigned char)(group3 >> 8);
    locator.address[7] = (unsigned char)group3;
    locator.address[8] = (unsigned char)(group4 >> 8);
    locator.address[9] = (unsigned char)group4;
    locator.address[10] = (unsigned char)(group5 >> 8);
    locator.address[11] = (unsigned char)group5;
    locator.address[12] = (unsigned char)(group6 >> 8);
    locator.address[13] = (unsigned char)group6;
    locator.address[14] = (unsigned char)(group7 >> 8);
    locator.address[15] = (unsigned char)group7;
    return true;
}
    ...
    const std::string &ipv6)
{
    ...
    {
        // LOG_WARN(IP_LOCATOR, "IPv6 " << ipv6 << " is not well defined");
        return false;
    }

    LOCATOR_ADDRESS_INVALID(locator.address);
    uint16_t count = (uint16_t)std::count_if(ipv6.begin(), ipv6.end(), [](char c)
                                                { return c == ':'; }); // C type cast to avoid Windows warnings
    ...
    if (initial_zeros != 0)
    {
        ...
        for (; i < 16;)
        {
            ...
            if (input_aux >= 65536)
            {
                // LOG_WARN(IP_LOCATOR, "IPv6 " << ipv6 << " has values higher than expected (65536)");
                return false;
            }
            locator.address[i++] = (input_aux >> 8);
            locator.address[i++] = (input_aux & 0xFF);
        }
    ...
    else
    {
        // No zero block
        ss >> input_aux;
        locator.address[0] = (input_aux >> 8);
        locator.address[1] = (input_aux & 0xFF);
        for (i = 2; i < 16;)
        {
            ss >> punct >> input_aux;
            if (input_aux >= 65536)
...
const unsigned char *IPLocator::getIPv6(
    const Locator &locator)
{
    ...
    const std::string &wan)
{
    ...
    if (ss >> a >> ch >> b >> ch >> c >> ch >> d)
    {
        locator.address[8] = (unsigned char)a;
        locator.address[9] = (unsigned char)b;
        locator.address[10] = (unsigned char)c;
        locator.address[11] = (unsigned char)d;
        return true;
    }
    return false;
}

const unsigned char *IPLocator::getWan(
    const Locator &locator)
{
    return static_cast<const unsigned char *>(&locator.address[8]);
}
    ...
    const std::string &lanId)
{
    if (locator.kind == LOCATOR_KIND_TCPv4)
    {
        ...
        {
            locator.address[0] = (unsigned char)a;
            locator.address[1] = (unsigned char)b;
            locator.address[2] = (unsigned char)c;
            locator.address[3] = (unsigned char)d;
            locator.address[4] = (unsigned char)e;
            locator.address[5] = (unsigned char)f;
            locator.address[6] = (unsigned char)g;
            locator.address[7] = (unsigned char)h;
...
const unsigned char *IPLocator::getLanID(
    const Locator &locator)
{
    return static_cast<const unsigned char *>(&locator.address[0]);
}
...
Locator IPLocator::toPhysicalLocator(
    const Locator &locator)
{
...
// Common
bool IPLocator::setPortTRANSPORT(
    Locator &locator,
    uint16_t port)
{
...
uint16_t IPLocator::getPortTRANSPORT(
    Locator &locator)
{
    if (locator.kind == LOCATOR_KIND_UDPv4 || locator.kind == LOCATOR_KIND_UDPv6)
    {
    ...
    const Locator &locator)
{
    if (locator.kind == LOCATOR_KIND_UDPv4 || locator.kind == LOCATOR_KIND_TCPv4)
    {
        return locator.address[12] == 127 && locator.address[13] == 0 && locator.address[14] == 0 && locator.address[15] == 1;
    }
    else
    {
        return locator.address[0] == 0 && locator.address[1] == 0 && locator.address[2] == 0 && locator.address[3] == 0 && locator.address[4] == 0 && locator.address[5] == 0 && locator.address[6] == 0 && locator.address[7] == 0 && locator.address[8] == 0 && locator.address[9] == 0 && locator.address[10] == 0 && locator.address[11] == 0 && locator.address[12] == 0 && locator.address[13] == 0 && locator.address[14] == 0 && locator.address[15] == 1;
    }
}
...
//         LOG_WARN(IP_LOCATOR, "Error " << ec.message() << " when execution the DNS request");
...
//     for (; it != end_it; ++it)
//     {
//         LOG_INFO(IP_LOCATOR, "IP " << it->endpoint().address() << " found by DNS request to address " << address_name);
...
std::string IPLocator::getIpByLocatorv4(const Locator &locator)
{
    char ip_str[16];
    sprintf(ip_str, "%d.%d.%d.%d", locator.address[12], locator.address[13], locator.address[14], locator.address[15]);
