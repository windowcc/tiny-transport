    ...
    bool return_loopback)
{
    ...
    if (rv != ERROR_SUCCESS)
    {
        // LOG_WARN(UTILS, "GetAdaptersAddresses() failed");
    ...
    bool return_loopback)
{
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (family == AF_INET)
        {
            ...
            if (s != 0)
            {
                // LOG_WARN(UTILS, "getnameinfo() failed: " << gai_strerror(s));
                continue;
            }
            info_IP info;
            info.type = IP4;
    ...
    std::vector<info_MAC> *macs)
{
    ...
    for (auto &ip : ips)
    {
        ...
        if (fd == -1)
        {
            // LOG_WARN(UTILS, "Error creating socket: " << strerror(errno));
            return false;
        }

        if (ioctl(fd, SIOCGIFHWADDR, &ifr) == -1)
        {
            // LOG_WARN(UTILS, "Error on ioctl: " << strerror(errno));
            close(fd);
            return false;
        }
    ...
    std::vector<info_MAC> *macs)
{
    ...
    {
        // LOG_WARN(UTILS, "getifaddrs() failed: " << strerror(errno));
        return false;
    }
